...
QuickSortHelper::QuickSortHelper(int low, int high)
    : m_low(low), m_high(high), m_pivot(m_high), m_i(m_low - 1), m_j(m_low),
      m_done(false) {}

Algorithm::Algorithm(std::string name, std::string time_complexity,
                     std::string space_complexity,
                     bool (*function)(Marker &, Algorithm &))
    : m_name(name), m_time_complexity(time_complexity),
      m_space_complexity(space_complexity), m_func(function) {
  reset_map();
}
